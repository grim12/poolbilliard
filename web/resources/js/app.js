...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.c-faq').forEach((group) => {
    ...
    toggles.forEach((toggle) => {
      ...
      // Markup can mark an item pre-opened (aria-expanded="true") — panel starts at max-h-0 in
      // CSS regardless, so expand it to its measured height once.
  ...
  // Gallery lightbox (GLightbox, loaded only on pages that have a .glightbox gallery) — library
  // isn't installed in web/ yet, so this is currently always a no-op.
