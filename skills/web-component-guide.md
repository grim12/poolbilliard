...
* **`Banner`** (`title`, `text`, `image`, `tag_text`, `meta_text`, `button_text`, `button_url`, `sort_order`) — zrcadlí `ui/`'s `eventBanner()` widget (jediný reálný příklad je "Federal Cup 2026" z `index.njk`). Žádný `is_active`/stavový sloupec a žádný slug/veřejná stránka — `Banner` je čistě obsahová knihovna k výběru, **umístění na homepage (které ze 2 "děr" ukazuje který banner, nebo žádný) bude řešit budoucí `HomepageSettings`** (nullable `banner_slot_1_id`/`banner_slot_2_id`, vybírané přes Filament `Select`), ne vlastnost samotného Banneru — vyhne se to nejednoznačnosti "víc aktivních najednou" u boolean flagu. Zbytek homepage widgetů/entit (`leaderboards`/žebříčky mj.) je zatím odložený, viz zpětný odkaz na tuhle diskuzi v konverzaci.
